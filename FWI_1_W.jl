...
# W2
opt_fn(x) = obj_fn_sinkhorn_parallel(data0-datac, x, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; cutoff=datac, pml_len=10, pml_coef=100, reg=5e-3, reg_m=1e2, reg_p=0, iterMax=100, verbose=false)

x0 = c[:]
xk, fn_value = gradient_descent(opt_fn, x0, 0.05, 20, 1, 1.05; rho=0.5, c=1e-5, maxSearchTime=5, threshold=1e-5)
matwrite("FWI_1_w2_gd.mat", Dict("xx" => xk,"ff" => fn_value,"c"=>c,"c0"=>c0); compress = false)

xk, fn_value = nonlinear_cg(opt_fn, x0, 0.05, 20, 1, 1.05; rho=0.5, c=1e-5, maxSearchTime=5, threshold=1e-5)
matwrite("FWI_1_w2_cg.mat", Dict("xx" => xk,"ff" => fn_value,"c"=>c,"c0"=>c0); compress = false)