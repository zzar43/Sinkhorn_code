function adj_source_sinkhorn(data1, data2; lambda=1000, numItermax=1000, stopThr = 1e-7, verbose=false);
    ...
    if length(size(data1)) == 2
        for i = 1:size(data1,2)
            f = data1[:,i]
            g = data2[:,i]
            T, adj[:,i], d1 = sinkhorn_signal_1d(f, g, M; lambda=lambda, numItermax=numItermax, stopThr=stopThr, verbose=verbose);
            dist += d1
        end
...
function adj_source_sinkhorn_parallel(data1, data2; lambda=1000, numItermax=1000, stopThr = 1e-7, verbose=false);
    Nt = size(data1,1)
    adj = 0 .* data1;
    ...
    if length(size(data1)) == 2
        @sync @distributed for i = 1:size(data1,2)
            f = data1[:,i]
            g = data2[:,i]
            T, adj[:,i], dist[1,i] = sinkhorn_signal_1d(f, g, M; lambda=lambda, numItermax=numItermax, stopThr=stopThr, verbose=verbose);
        end
    elseif length(size(data1)) == 3
        @sync @distributed for i = 1:size(data1,2)
            for j = 1:size(data1,3)
