...
function eval_grad_OT(data0, c, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; save_ratio=1, pml_len=10, pml_coef=100, reg=5e-3, reg_m=1e2, reg_p=0, iterMax=1000, cutoff=0)
    x = reshape(c, Nx, Ny)
    if minimum(x) > 0
        data, u = multi_solver_parallel(x, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; save_ratio=save_ratio, pml_len=pml_len, pml_coef=pml_coef)

        gradk, fk = grad_sinkhorn_parallel(data, u, data0, x, rho, Nx, Ny, Nt, h, dt, source_position, receiver_position; reg_p=reg_p, pml_len=pml_len, pml_coef=pml_coef, reg=reg, reg_m=reg_m, iterMax=iterMax, verbose=false, save_ratio=save_ratio)
...
function eval_fn_OT(data0, c, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; save_ratio=1, pml_len=10, pml_coef=100, reg=5e-3, reg_m=1e2, reg_p=0, iterMax=1000)
    x = reshape(c, Nx, Ny)
    if minimum(x) > 0
        t = range(0,step=dt,length=Nt)
        M = cost_matrix_1d(t,t)
        data, u = multi_solver_parallel(x, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; save_ratio=save_ratio, pml_len=pml_len, pml_coef=pml_coef)
        adj_source, fk = adj_source_sinkhorn_parallel(data, data0, M; reg_p=reg_p, reg=reg, reg_m=reg_m, iterMax=iterMax, verbose=false);
    else
        fk = NaN
        print(" Wrong update, with entries of x <= 0. ")
    end
